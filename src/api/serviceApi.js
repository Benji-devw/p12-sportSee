...
function getUrl(fileName, id, slug) {
    if (import.meta.env.MOCK) {
        ...
        return `${import.meta.env.VITE_DEV_API_URL}/user/${id}${slug?`/${slug}`:''}`;
    }
}
...
export const getUser = async (id) => {
    ...
    const user = new User(data.data.id, data.data.userInfos.firstName, data.data.score);
...
export const getPerformances = async (id) => {
    ...
    return data.data.data.map((performance) => {
        return new Performence(data.data.kind[performance.kind], performance.value);
    });
}
...
export const getActivities = async (id) => {
    ...
    return data.data.sessions.map(activity => {
        return new ActivitySession(activity.day, activity.kilogram, activity.calories);
    });
}

export const getSessionAverage = async (id) => {
    const response = await fetch(getUrl('getActivities', id, 'average-sessions'));
    if (!response.ok) {
        throw new Error(`API call failed with status ${response.status}`);
    }
    const data = await response.json();
